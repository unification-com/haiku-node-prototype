...
class UnificationACLValidation:
    ...
    def app_has_user_permission(self, user_account):
        ...
        for i in self.granted:
            if int(i) == user_acc_uint64:
                has_permission = True
    ...
    def check_acl_contract_code_hash(self, hash):
        code_valid = False
    ...
    def _get_app_permissions(self):
        self.granted = []
        self.revoked = []
        table_data = self.eosClient.get_table_rows(self.requesting_app, self.conf['acl_contract'], "unifacl", True, 0, -1, -1)
